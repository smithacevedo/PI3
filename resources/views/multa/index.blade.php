...
<table class="multa-table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <button class="btn btn-primary" onclick="window.location='{{ url('/multa/'.$multa->id.'/edit')}}'">Editar</button>
                <form action="{{ url('/multa/'.$multa->id) }}" method="post" style="display: inline-block;">
                    @csrf
                    @method('DELETE')
                    <input class="btn btn-primary" type="submit" onclick="return confirm('¿Esta multa ya fué pagada?')" value="Pagada">
                </form>
            </td>
        </tr>
        @endforeach
