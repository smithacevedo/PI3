...
<table class="book-table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <button class="btn btn-primary" onclick="window.location='{{ url('/autor/'.$autor->id.'/edit')}}'">Editar</button>
                <form action="{{ url('/autor/'.$autor->id) }}" method="post" style="display: inline-block;">
                    @csrf
                    {{ method_field('DELETE') }}
                    <input class="btn btn-primary" type="submit" onclick="return confirm('¿Seguro que quieres eliminar este autor?')" value="Borrar">
                </form>
            </td>
        </tr>
        @endforeach
